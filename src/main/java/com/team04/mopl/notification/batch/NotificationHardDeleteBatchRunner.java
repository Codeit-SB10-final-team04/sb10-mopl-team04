...
public class NotificationHardDeleteBatchRunner {

	// Spring Batch Job을 실행하는 객체
	private final JobLauncher jobLauncher;

	// NotificationHardDeleteJobConfig에서 `@Bean`으로 등록한 Job
	private final Job notificationHardDeleteJob;
	...
	public void run(LocalDate deleteDate) {
		try {
			JobParameters jobParameters = new JobParametersBuilder()
				...
				// 스케줄 실행마다 새로운 JobInstance로 실행되도록 runId 추가
				.addLong("runId", System.currentTimeMillis())
				// Builder에서 만든 값을 JobParameter에 추가
				.toJobParameters();

			log.info("[NOTIFICATION_HARD_DELETE_BATCH] 알림 물리 삭제 배치 시작: deleteDate={}", deleteDate);

			JobExecution jobExecution = jobLauncher.run(notificationHardDeleteJob, jobParameters);

			if (jobExecution.getStatus() != BatchStatus.COMPLETED) {
				throw new IllegalStateException("알림 물리 삭제 배치 실패: jobExecutionId="
					+ jobExecution.getId() + ", status=" + jobExecution.getStatus());
			}

			log.info("[NOTIFICATION_HARD_DELETE_BATCH] 알림 물리 삭제 배치 성공: deleteDate={}", deleteDate);
		} catch (Exception e) {
			log.error("[NOTIFICATION_HARD_DELETE_BATCH] 알림 물리 삭제 배치 실패: deleteDate={}",
				deleteDate, e);
			throw new BatchException("알림 물리 삭제 배치에 실패했습니다.", e);
		}
	}
}
