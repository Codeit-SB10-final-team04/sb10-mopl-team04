...
public class PlaylistQdslRepositoryImpl implements PlaylistQdslRepository {
	...
	@Override
	public PlaylistCursorPage findAllPlaylists(PlaylistPageRequest request) {
	...
	private BooleanExpression updateAtCursorCondition(
		String cursor,
	...
	private BooleanExpression updateAtCursor(
		SortDirection sortDirection,
		UUID idAfter,
	...
	private OrderSpecifier<?>[] orderCondition(
		...
		if (sortBy.equals(PlaylistSortBy.updatedAt)) {
			return updatedAtOrder(sortDirection);
		}
	...
	// 조건에 따른 전체 데이터 개수 조회
	private Long getTotalCount(PlaylistPageRequest request) {
