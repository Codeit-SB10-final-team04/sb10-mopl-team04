...
class FollowServiceTest {
	...
	@Test
	@DisplayName("성공: Redis 캐시에 팔로워가 0명이지만 DB에 존재하면 DB 값을 반환한다.")
	void getFollowerCount_CacheMiss_DBHasCount_Success() {
		...
		// Redis Cache Miss (0L)
		given(followRedisStore.getFollowerCount(followeeId)).willReturn(0L);
	...
	@Test
	@DisplayName("성공: Redis와 DB 모두 팔로워가 0명이면 0을 정상 반환한다.")
